...
- **Exactly one identity can push to main: choirmaster's credential.** Enforced by branch protection — a lock with one keyholder. The remote enforces only this lock; the checks live in the landing script below. The companion has no git identity; anything it produces lands under choirmaster's credential with the companion's attribution stamped in the commit message.
